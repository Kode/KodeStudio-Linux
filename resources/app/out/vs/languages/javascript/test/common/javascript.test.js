 ...
 *--------------------------------------------------------------------------------------------*/
'use strict';
define(["require", "exports", 'assert', 'vs/editor/test/common/modesUtil'], function (require, exports, assert, modesUtil) {
    suite('JS - Auto Indent', function () {
        var wordDefinition;
        var onEnter;
        var assertOnEnter;
        var onElectricCharacter;
        var assertWords = modesUtil.assertWords;
        suiteSetup(function (done) {
            modesUtil.load('javascript').then(function (mode) {
        ...
        test('Word definition', function () {
            ...
            assertWords('modesRegistry.registerMode([\'text/x-monaco-buildlog\'], new Platform.DeferredDescriptor(\'vs/workbench/contrib/build/buildViewlet\', \'BuildLogMode\'));'.match(wordDefinition), ['modesRegistry', 'registerMode', 'text', 'x', 'monaco', 'buildlog', 'new', 'Platform', 'DeferredDescriptor', 'vs', 'workbench', 'contrib', 'build',
                'buildViewlet', 'BuildLogMode']);
            assertWords('var BuildDataSource = winjs.Class.define(function BuildDataSource() {}, {'.match(wordDefinition), ['var', 'BuildDataSource', 'winjs', 'Class', 'define', 'function', 'BuildDataSource']);
            assertWords('		} else if(element instanceof BuildModel.BuildError) {'.match(wordDefinition), ['else', 'if', 'element', 'instanceof', 'BuildModel', 'BuildError']);
        });
    });
});
