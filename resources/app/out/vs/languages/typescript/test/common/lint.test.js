 ...
 *--------------------------------------------------------------------------------------------*/
'use strict';
define(["require", "exports", 'assert', 'vs/base/common/types', 'vs/languages/typescript/common/lib/typescriptServices', 'vs/languages/typescript/common/lint/lint', 'vs/languages/typescript/common/lint/rules/layout', 'vs/languages/typescript/common/lint/rules/javascript', 'vs/languages/typescript/common/lint/rules/typescript', './features/utils', 'vs/editor/common/core/range'], function (require, exports, assert, types, ts, lint, layout, javascript, typescript, utils, range_1) {
    function _assertCheck(checker, sourceFile, numbers) {
        var expected = [];
        ...
        while (actual.length > 0) {
            var _expected = expected.pop(), _actual = actual.pop().range;
            assert.ok(range_1.Range.equalsRange(_expected, _actual), JSON.stringify(_expected) + JSON.stringify(_actual));
        }
    }
    function assertCheck3(ctor, source) {
        var numbers = [];
    ...
    function assertCheck2(ctor, source) {
        var numbers = [];
        for (var _i = 2; _i < arguments.length; _i++) {
            numbers[_i - 2] = arguments[_i];
        }
        var checker = new lint.SimpleStyleRuleChecker([{ rule: types.create(ctor), severity: 1 }]);
        var sourceFile = ts.createSourceFile('a.ts', source, 1 /* ES5 */);
        ts.bindSourceFile(sourceFile);
        _assertCheck(checker, sourceFile, numbers);
    }
    suite('TypeScript - lint', function () {
