 ...
 *--------------------------------------------------------------------------------------------*/
'use strict';
define(["require", "exports", 'assert', 'vs/base/common/winjs.base', 'vs/languages/json/common/jsonSchemaService', 'vs/languages/json/common/parser/jsonParser', 'vs/base/node/pfs', 'path'], function (require, exports, assert, winjs, SchemaService, Parser, pfs, path) {
    suite('JSON - schema', function () {
        var fixureDocuments = {
        ...
        var requestServiceMock = {
            makeRequest: function (options) {
                ...
                if (fileName) {
                    ...
                    return pfs.readFile(path.join(fixtures, fileName)).then(function (value) {
        ...
        test('Resolving $refs', function (testDone) {
            ...
            service.getResolvedSchema('https://myschemastore/main').then(function (fs) {
        ...
        test('FileSchema', function (testDone) {
            ...
            service.getResolvedSchema('main').then(function (fs) {
        ...
        test('Preloaded Schema', function (testDone) {
            ...
            service.getSchemaForResource('test.json', null).then(function (schema) {
        ...
        test('Resolving in-line $refs', function (testDone) {
            ...
            service.registerExternalSchema(id, ['*.json'], schema);
            service.getSchemaForResource('test.json', null).then(function (fs) {
                var section = fs.getSection(['child', '0', 'grandchild']);
                assert.equal(section.description, 'Meaning of Life');
            }).done(function () { return testDone(); }, function (error) {
                testDone(error);
        ...
        test('Resolving in-line $refs automatically for external schemas', function (testDone) {
            ...
            fsm.getResolvedSchema().then(function (fs) {
                var section = fs.getSection(['child', '0', 'grandchild']);
                assert.equal(section.description, 'Meaning of Life');
            }).done(function () { return testDone(); }, function (error) {
                testDone(error);
        ...
        test('Resolving circular $refs', function (testDone) {
            ...
            service.getSchemaForResource('file://doc/mydoc.json', document).then(function (resolveSchema) {
        ...
        test('Validate Azure Resource Dfinition', function (testDone) {
            ...
            var parser = new Parser.JSONParser();
            var document = parser.parse(JSON.stringify(input));
            service.getSchemaForResource('file://doc/mydoc.json', document).then(function (resolvedSchema) {
                assert.deepEqual(resolvedSchema.errors, []);
