    ...
    var JSONIntellisense = (function () {
                ...
                if (s.node === node && !s.inverted) {
                    ...
                    if (schemaProperties) {
                        Object.keys(schemaProperties).forEach(function (key) {
                            ...
                            collector.add({ type: 'property', label: key, codeSnippet: _this.getSnippetForProperty(key, propertySchema, addValue, isLast), documentationLabel: propertySchema.description || '' });
                        });
                    }
                }
            });
        ...
        JSONIntellisense.prototype.getSchemaLessPropertySuggestions = function (doc, node, collector) {
            ...
            var collectSuggestionsForSimilarObject = function (obj) {
                obj.properties.forEach(function (p) {
                    var key = p.key.value;
                    collector.add({ type: 'property', label: key, codeSnippet: _this.getSnippetForSimilarProperty(key, p.value), documentationLabel: '' });
                });
            };
        ...
        JSONIntellisense.prototype.getSchemaLessValueSuggestions = function (doc, node, offset, modelMirror, collector) {
            ...
            var collectSuggestionsForValues = function (value) {
                var content = _this.getMatchingSnippet(value, modelMirror);
        ...
        JSONIntellisense.prototype.addEnumSuggestion = function (schema, collector) {
            ...
            if (Array.isArray(schema.enum)) {
                schema.enum.forEach(function (enm) { return collector.add({ type: _this.getSuggestionType(schema.type), label: _this.getLabelForValue(enm), codeSnippet: _this.getSnippetForValue(enm), documentationLabel: '' }); });
            }
        ...
        JSONIntellisense.prototype.getSnippetForValue = function (value) {
        ...
        JSONIntellisense.prototype.getMatchingSnippet = function (node, modelMirror) {
            switch (node.type) {
                case 'array':
                    return '[]';
                case 'object':
        ...
        JSONIntellisense.prototype.getSnippetForProperty = function (key, propertySchema, addValue, isLast) {
            var result = '"' + key + '"';
            if (!addValue) {
                return result;
            }
        ...
        JSONIntellisense.prototype.getSnippetForSimilarProperty = function (key, templateValue) {
            return '"' + key + '"';
        };
