    ...
    suite('FileService', function () {
        ...
        setup(function (done) {
            ...
            extfs.copy(sourceDir, testDir, function () {
                ...
                service = new fileService_1.FileService(testDir, events, { disableWatcher: true });
                done();
            });
        });
        ...
        test('updateContent - use encoding (UTF 16 BE)', function (done) {
            ...
            service.resolveContent(resource).done(function (c) {
                c.charset = charset;
                return service.updateContent(c.resource, c.value, { charset: charset }).then(function (c) {
                    return async_1.nfcall(encoding.detectEncodingByBOM, c.resource.fsPath).then(function (enc) {
        ...
        test('resolveContent - encoding picked up', function (done) {
            var resource = uri_1.default.file(path.join(testDir, 'index.html'));
            var charset = 'windows1252';
            service.resolveContent(resource, { encoding: charset }).done(function (c) {
                assert.equal(c.charset, charset);
                done();
            });
        });
        ...
        test('options', function (done) {
            ...
            extfs.copy(_sourceDir, _testDir, function () {
                ...
                var _service = new fileService_1.FileService(_testDir, null, {
