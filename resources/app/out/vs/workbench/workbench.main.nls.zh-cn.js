...
define("vs/workbench/workbench.main.nls.zh-cn", {
	...
	"vs/editor/node/textMate/TMSyntax": [
		...
		"\"contributes.{0}.embeddedLanguages\" 中的无效值。必须为从作用域名称映射到语言的对象。给定值: {1}",
		"“contributes.{0}.path”({1})应包含在扩展的文件夹({2})内。这可能会使扩展不可移植。",
	],
	...
	"vs/workbench/browser/actions/openSettings": [
		...
		"通过将设置放入设置文件中来覆盖设置。",
		"请参阅 http://go.microsoft.com/fwlink/?LinkId=808995 以了解最常用的设置。",
		"默认设置",
	...
	"vs/workbench/parts/debug/node/debugAdapter": [
		...
		"配置名称；在启动配置下拉菜单中显示。",
		"请求配置类型。可以是“启动”或“附加”。",
		"要在此“复合”配置期间启动的配置。仅考虑此配置类型为“复合”时。",
		"调试会话开始前要运行的任务。",
		"内部调试控制台的控制行为。",
		"特定于 Windows 的启动配置属性。",
		"特定于 OS X 的启动配置属性。",
	...
	"vs/workbench/parts/files/browser/fileActions": [
		...
		"重命名",
		"重命名",
		"你正在重命名的文件夹有 1 个文件具有未保存的更改。是否继续?",
		"你正在重命名的文件夹有 {0} 个文件具有未保存的更改。是否继续?",
		"你正在重命名的文件具有未保存的更改。是否继续?",
		"如果不保存，更改将丢失。",
		"重命名(&&R)",
		"新建文件",
		"新建文件夹",
	...
	"vs/workbench/parts/files/browser/views/explorerViewer": [
		...
		"{0}，文件资源管理器",
		"你正在移动的文件夹有 1 个文件具有未保存的更改。是否继续?",
		"你正在移动的文件夹有 {0} 个文件具有未保存的更改。是否继续?",
		"你正在移动的文件具有未保存的更改。是否继续?",
		"如果不保存，更改将丢失。",
		"移动(&&M)",
		"目标文件夹中已存在“{0}”。是否要将其替换?",
		"此操作不可逆!",
	...
	"vs/workbench/parts/markers/common/messages": [
		...
		"({0}，{1})",
		"含有 {1} 问题的 {0}",
		"由第 {2} 行、第 {3} 列的 {0}: {1} 生成的错误",
		"错误: 第 {1} 行、第 {2} 列的 {0}",
		"由第 {2} 行、第 {3} 列的 {0}: {1} 生成的浸警告",
		"警告: 第 {1} 行、第 {2} 列的 {0}",
		"由第 {2} 行、第 {3} 列的 {0}: {1} 生成的信息",
		"信息: 第 {1} 行、第 {2} 列的 {0}",
		"由第 {2} 行、第 {3} 列的 {0}: {1} 生成的问题",
		"问题: 第 {1} 行、第 {2} 列的 {0}",
		"错误和警告",
	],
	...
	"vs/workbench/parts/tasks/electron-browser/task.contribution": [
		...
		"存在运行中的任务。要终止它吗?",
		"终止任务(&&T)",
		"启动的任务不再存在。如果任务衍生的后台进程退出 VS Code，则可能会导致出现孤立的进程。若要避免此情况，请使用等待标记启动最后一个后台进程。",
		"仍然退出(&&E)",
		...
		"问题位置的匹配组索引。有效的位置模式为(line)、(line,column)和(startLine,startColumn,endLine,endColumn)。如果省略了，将假定行和列。",
		"问题行的匹配组索引。默认值为 2",
		"问题列的匹配组索引。默认值为 3",
		"问题结束行的匹配组索引。默认为未定义",
		"问题结束列的匹配组索引。默认为未定义",
	...
	"vs/workbench/services/themes/electron-browser/themeService": [
		...
		"关联的图标定义的 ID。",
		"将文件扩展名关联到图标。对象键是文件扩展名。扩展名是文件名的最后一个点(不包括句点)的最后一部分。扩展名不区分大小写，以便进行比较。",
		"关联的图标定义的 ID。",
