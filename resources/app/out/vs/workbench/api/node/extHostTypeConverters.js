    ...
    (function (Command) {
        ...
        function from(command, context) {
            ...
            if (!arrays_1.isFalsyOrEmpty(command.arguments)) {
                // keep command around
                var id = command.command + "-no-args-wrapper-" + _idPool++;
                result.id = id;
                _cache[id] = command;
                var disposable1 = context.commands.registerCommand(id, function () { return (_a = context.commands).executeCommand.apply(_a, [command.command].concat(_cache[id].arguments)); var _a; });
                var disposable2 = { dispose: function () { delete _cache[id]; } };
