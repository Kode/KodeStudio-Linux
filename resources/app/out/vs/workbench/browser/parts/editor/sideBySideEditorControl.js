    ...
    var SideBySideEditorControl = (function (_super) {
        ...
        SideBySideEditorControl.prototype.makeSplitEditorAction = function () {
            ...
            return new actions_1.Action('split.editor.action', nls.localize('splitEditor', "Split Editor"), 'split-editor-action', true, function () {
                ...
                if (editorCount > 1 && activeEditor.position === editor_2.Position.LEFT) {
                    var centerInput = _this.visibleEditors[editor_2.Position.CENTER].input;
                    var options = new editor_1.TextEditorOptions();
                    options.preserveFocus = true;
                    return _this.editorService.openEditor(activeEditor.input, options, editor_2.Position.CENTER).then(function () {
        ...
        SideBySideEditorControl.prototype.trackFocus = function (editor, position) {
            ...
            if (!this.globalFocusTracker) {
                ...
                this.globalFocusTracker.addBlurListener(function () {
                    var activeElement = window.document.activeElement;
                    if (activeElement) {
                        for (var i = 0; i < _this.visibleEditorContainers.length; i++) {
                            var container = _this.visibleEditorContainers[i];
                            if (container && DOM.isAncestor(activeElement, container.getHTMLElement())) {
                                _this.onFocusGained(_this.visibleEditors[i]);
                                break;
                            }
                        }
                    }
        ...
        SideBySideEditorControl.prototype.setActive = function (editor) {
            ...
            if (this.lastActiveEditor !== editor) {
                ...
                if (this.containerWidth[this.lastActivePosition] === SideBySideEditorControl.MIN_EDITOR_WIDTH) {
                    ...
                    var remainingWidth = this.dimension.width;
                    ...
                    editor_2.POSITIONS.forEach(function (p) {
                        if (_this.lastActivePosition !== p && !!_this.visibleEditors[p]) {
                            ...
                            remainingWidth -= _this.containerWidth[p];
                        }
                    });
        ...
        SideBySideEditorControl.prototype.focusNextNonMinimized = function () {
            ...
            if (!types.isUndefinedOrNull(this.lastActivePosition) && this.containerWidth[this.lastActivePosition] === SideBySideEditorControl.MIN_EDITOR_WIDTH) {
                var candidate = null;
                var currentWidth = SideBySideEditorControl.MIN_EDITOR_WIDTH;
                editor_2.POSITIONS.forEach(function (position) {
                    ...
                    if (position !== _this.lastActivePosition) {
                        if (_this.visibleEditors[position] && _this.containerWidth[position] > currentWidth) {
                            candidate = position;
                            currentWidth = _this.containerWidth[position];
                        }
                    }
                });
        ...
        SideBySideEditorControl.prototype.getWidthRatios = function () {
            ...
            var ratio = [];
            if (this.dimension) {
                var fullWidth = this.dimension.width;
                editor_2.POSITIONS.forEach(function (position) {
                    if (_this.visibleEditors[position]) {
                        ratio.push(_this.containerWidth[position] / fullWidth);
                    }
                });
            }
            return ratio;
        ...
        SideBySideEditorControl.prototype.create = function (parent) {
            var _this = this;
            // Left Container
            this.containers[editor_2.Position.LEFT] = builder_1.$(parent).div({ class: 'one-editor-container editor-left' });
            // Left Sash
            ...
            this.leftSash.hide();
            // Center Container
            this.containers[editor_2.Position.CENTER] = builder_1.$(parent).div({ class: 'one-editor-container editor-center' });
            // Right Sash
            ...
            this.rightSash.hide();
            // Right Container
            this.containers[editor_2.Position.RIGHT] = builder_1.$(parent).div({ class: 'one-editor-container editor-right' });
            // Title containers
            editor_2.POSITIONS.forEach(function (position) {
                _this.titleContainer[position] = builder_1.$(_this.containers[position]).div({ 'class': 'title monaco-editor-background' });
        ...
        SideBySideEditorControl.prototype.layoutControl = function (dimension) {
            ...
            if (totalWidth > 0) {
                var overflow = totalWidth - this.dimension.width;
                ...
                if (overflow < 0) {
                    ...
                    var positionToGive = null;
                    editor_2.POSITIONS.forEach(function (position) {
                        if (_this.visibleEditors[position] && positionToGive === null && _this.containerWidth[position] !== SideBySideEditorControl.MIN_EDITOR_WIDTH) {
                            positionToGive = position;
                        }
                    });
                    if (positionToGive === null) {
                        positionToGive = editor_2.Position.LEFT; // maybe all are minimized, so give LEFT the extra width
                    }
                    ...
                    editor_2.POSITIONS.forEach(function (position) {
                        ...
                        if (maxCompensation >= overflow) {
                            _this.containerWidth[position] -= overflow;
                            overflow = 0;
                        }
                        else if (maxCompensation > 0) {
                            var compensation = overflow - maxCompensation;
