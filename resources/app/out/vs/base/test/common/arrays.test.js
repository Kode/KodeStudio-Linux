...
define(["require", "exports", 'assert', 'vs/base/common/arrays'], function (require, exports, assert, arrays) {
    suite('Arrays', function () {
        test('findFirst', function () {
            ...
            var idx = arrays.findFirst(array, function (e) { return e > 1; });
            assert.equal(array[idx], 4);
            var idx = arrays.findFirst(array, function (e) { return e >= 8; });
            assert.equal(array[idx], 55);
            var idx = arrays.findFirst(array, function (e) { return e >= 61; });
            assert.equal(array[idx], 61);
            var idx = arrays.findFirst(array, function (e) { return e >= 69; });
            assert.equal(array[idx], 69);
            var idx = arrays.findFirst(array, function (e) { return e >= 70; });
            assert.equal(idx, array.length);
            var idx = arrays.findFirst([], function (e) { return e >= 0; });
            assert.equal(array[idx], 1);
        });
