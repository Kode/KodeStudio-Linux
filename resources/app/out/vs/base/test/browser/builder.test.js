...
define(["require", "exports", 'assert', 'vs/base/browser/builder', 'vs/base/common/types', 'vs/base/browser/dom', 'vs/base/common/winjs.base'], function (require, exports, assert, builder_1, Types, DomUtils, winjs_base_1) {
    ...
    suite("Builder", function () {
        ...
        test("Dimension.substract()", function () {
            var d1 = new builder_1.Dimension(200, 100);
            var d2 = new builder_1.Box(10, 20, 30, 40);
            assert.deepEqual(d1.substract(d2), new builder_1.Dimension(140, 60));
        });
        test("Position", function () {
            var p = new builder_1.Position(200, 100);
        ...
        test("Binding", function () {
            ...
            assert(element);
            // Properties
            builder_1.Binding.setPropertyOnElement(element, "foo", "bar");
            assert.strictEqual(builder_1.Binding.getPropertyFromElement(element, "foo"), "bar");
            builder_1.Binding.setPropertyOnElement(element, "foo", { foo: "bar" });
            assert.deepEqual(builder_1.Binding.getPropertyFromElement(element, "foo"), { foo: "bar" });
            builder_1.Binding.removePropertyFromElement(element, "foo");
            builder_1.Binding.setPropertyOnElement(element, "bar", "bar");
            assert.strictEqual(builder_1.Binding.getPropertyFromElement(element, "bar"), "bar");
            builder_1.Binding.setPropertyOnElement(element, "bar", { foo: "bar" });
            assert.deepEqual(builder_1.Binding.getPropertyFromElement(element, "bar"), { foo: "bar" });
            builder_1.Binding.removePropertyFromElement(element, "bar");
            assert(!builder_1.Binding.getPropertyFromElement(element, "foo"));
            assert(!builder_1.Binding.getPropertyFromElement(element, "bar"));
            // Binding
            builder_1.Binding.bindElement(element, "bar");
            assert.strictEqual(builder_1.Binding.getBindingFromElement(element), "bar");
            builder_1.Binding.bindElement(element, { foo: "bar" });
            assert.deepEqual(builder_1.Binding.getBindingFromElement(element), { foo: "bar" });
            builder_1.Binding.unbindElement(element);
            assert(!builder_1.Binding.getBindingFromElement(element));
        });
        ...
        test("Builder.and() with 2 Builders", function () {
            var b = builder_1.Build.withElementById(fixtureId);
            var otherB = builder_1.Build.withElementById(fixtureId);
        ...
        test("Builder.and() with MultiBuilder and HTMLElement", function () {
            ...
            var allDivsFixture = allDivs.and(builder_1.Build.withElementById(fixtureId).getHTMLElement());
        ...
        test("Builder Multibuilder fn call that returns Multibuilder", function () {
            ...
            var multiBuilder = builder_1.Build.withElementById(fixtureId).select("div");
        ...
        test("Builder.p() and other elements", function () {
            ...
            assert.strictEqual(builder_1.Build.withElementById(fixtureId).select("div").length, 1);
            assert.strictEqual(builder_1.Build.withElementById(fixtureId).select("*").length, 7);
        ...
        test("Builder.showDelayed()", function (done) {
            ...
            b.showDelayed(20);
            assert(b.hasClass("hidden"));
            winjs_base_1.Promise.timeout(30).then(function () {
                assert(!b.hasClass("hidden"));
                done();
            });
        });
        ...
        test("Builder.select() and .matches()", function () {
            ...
            assert(b.getPositionRelativeTo(builder_1.$(window.document.body)));
        ...
        test("Builder.empty()", function () {
            ...
            assert(builder_1.Build.withElementById(fixtureId).select("*").length === 0);
            inputs.forEach(function (input) {
                input.domClick();
            });
        ...
        test("Builder.destroy()", function () {
            ...
            builder_1.Build.withElementById(fixtureId).select("div").destroy();
            assert(builder_1.Build.withElementById(fixtureId).select("*").length === 0);
            inputs.forEach(function (input) {
                input.domClick();
            });
        ...
        test("Builder.empty() MultiBuilder", function () {
            ...
            builder_1.Build.withElementById(fixtureId).select("div > input").domClick();
            builder_1.Build.withElementById(fixtureId).select("div").empty();
            inputs.forEach(function (input) {
                input.domClick();
            });
        ...
        test("XSS - safeInnerHtml()", function () {
