 ...
 *--------------------------------------------------------------------------------------------*/
'use strict';
define(["require", "exports", 'assert', 'vs/editor/standalone-languages/powershell', 'vs/editor/standalone-languages/test/testUtil'], function (require, exports, assert, languageDef, T) {
    var Bracket = {
        Open: 1,
        Close: -1
    };
    ...
    suite('powershell', function () {
        test('word definition', function () {
            ...
            assert.deepEqual('Connect-XenServer -url $parameterSet["url"] <#-opaqueref trala#>")'.match(wordDefinition), ['Connect-XenServer', '-url', '$parameterSet', 'url', '-opaqueref', 'trala']);
            assert.deepEqual('$exp = "Get-XenServer:{0} -properties @{{uuid=\'{1}\'}}" -f $parameterSet["class"], $parameterSet["objUuid"]'.match(wordDefinition), ['$exp', 'Get-XenServer', '0', '-properties', 'uuid', '1', '-f', '$parameterSet', 'class', '$parameterSet', 'objUuid']);
        });
    });
});
