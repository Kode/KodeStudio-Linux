...
function analyze(contents, options) {
    ...
    // all imports
    var imports = collect(sourceFile, function (n) { return isRequireImport(n) ? CollectStepResult.YesAndRecurse : CollectStepResult.NoAndRecurse; });
    var nlsReferences = imports.reduce(function (memo, node) {
        if (node.kind === ts.SyntaxKind.CallExpression) {
            var parent = node.parent;
            if (isVariableDeclaration(parent)) {
                var references = service.getReferencesAtPosition(filename, parent.name.pos + 1);
            ...
            service.getReferencesAtPosition(filename, parent.name.pos + 1).forEach(function (reference) {
                if (!reference.isWriteAccess) {
                    ...
                    if (isIdentifier(node)) {
                        ...
                        if (isCallExpression(parent_1) && parent_1.arguments.length >= 2) {
                            memo.push(parent_1);
                        }
                        else {
...
function processFile(contents, sourceMap) {
    var analysisResult = analyze(contents);
...
function createLocalizedMessages(filename, bundle, language, i18nBaseDir, baseDir) {
    var problems = [];
    ...
    if (ResolvedJavaScriptMessageBundle.is(bundle)) {
        return { messages: ResolvedJavaScriptMessageBundle.asTranslatedMessages(bundle, messages, problems), problems: problems };
    }
    else {
        return { messages: PackageJsonMessageBundle.asTranslatedMessages(bundle, messages, problems), problems: problems };
    }
}
