...
var NodeDebugSession = (function (_super) {
    ...
    NodeDebugSession.prototype.launchRequest = function (response, args) {
        ...
        if (runtimeExecutable) {
            ...
            if (!terminal_1.Terminal.isOnPath(NodeDebugSession.NODE)) {
                this.sendErrorResponse(response, 2001, localize(9, null, '{_runtime}'), { _runtime: NodeDebugSession.NODE });
                return;
            }
        ...
        if (NodeDebugSession.isJavaScript(programPath)) {
            ...
            if (!this._sourceMaps) {
                this.sendErrorResponse(response, 2002, localize(11, null, '{path}'), { path: programPath });
                return;
            }
            this._sourceMaps.MapPathFromSource(programPath).then(function (generatedPath) {
                if (!generatedPath) {
                    _this.sendErrorResponse(response, 2003, localize(12, null, '{path}', 'outDir'), { path: programPath });
                    return;
                }
                _this.log('sm', "launchRequest: program '" + programPath + "' seems to be the source; launch the generated file '" + generatedPath + "' instead");
                programPath = generatedPath;
    ...
    NodeDebugSession.prototype.launchRequest2 = function (response, args, programPath, programArgs, runtimeExecutable, runtimeArgs, port) {
        ...
        if (this._externalConsole) {
            terminal_1.Terminal.launchInTerminal(workingDirectory, launchArgs, args.env).then(function (term) {
                ...
                _this.sendErrorResponseWithInfoLink(response, 2011, localize(13, null, '{_error}'), { _error: error.message }, error.linkId);
            ...
            nodeProcess.on('error', function (error) {
                _this.sendErrorResponse(response, 2017, localize(14, null, '{_error}'), { _error: error.message }, vscode_debugadapter_1.ErrorDestination.Telemetry | vscode_debugadapter_1.ErrorDestination.User);
        ...
        if (typeof args.processId === 'string') {
            ...
            if (/^([0-9]+)$/.test(pid_string)) {
                ...
                try {
                    ...
                    this.sendErrorResponse(response, 2021, localize(15, null, pid, e));
                    return;
                }
            }
            else {
                this.sendErrorResponse(response, 2006, localize(16, null, pid_string));
                return;
            }
        }
            ...
            if (connected) {
                ...
                if (err.code === 'ECONNREFUSED' || err.code === 'ECONNRESET') {
                    ...
                    if (now < endTime) {
                        ...
                        _this.sendErrorResponse(response, 2009, localize(17, null, '{_timeout}'), { _timeout: timeout });
                    }
                }
                else {
                    _this.sendErrorResponse(response, 2010, localize(18, null, '{_error}'), { _error: err.message });
                }
            }
        });
    ...
    NodeDebugSession.prototype._startInitialize2 = function (stopped) {
        ...
        if (this._stopOnEntry) {
            ...
            this.sendEvent(new vscode_debugadapter_1.StoppedEvent(localize(19, null), NodeDebugSession.DUMMY_THREAD_ID));
        }
        else {
        ...
        if (!this._inShutdown) {
            ...
            if (this._attachMode) {
                ...
                if (pid > 0) {
                    ...
                    terminal_1.Terminal.killTree(pid);
                }
            }
        ...
        if (source.name) {
            ...
            this._findModule(source.name).then(function (scriptId) {
                if (scriptId >= 0) {
                    ...
                    _this.sendErrorResponse(response, 2019, localize(21, null, '{_module}'), { _module: source.name });
                }
                return;
            });
            return;
    ...
    NodeDebugSession.prototype.configurationDoneRequest = function (response, args) {
        ...
        if (this._needBreakpointEvent) {
            ...
            this.sendEvent(new vscode_debugadapter_1.StoppedEvent(localize(23, null), NodeDebugSession.DUMMY_THREAD_ID));
        }
        ...
        this._node.command2(cmd, backtraceArgs).then(function (response) {
            ...
            if (error.message === 'no stack') {
                if (_this._stoppedReason === 'pause') {
                    _this.sendErrorResponse(response, 2022, localize(24, null));
                }
                else {
                    _this.sendErrorResponse(response, 2023, localize(25, null));
                }
            }
            else {
                _this.sendErrorResponse(response, 2018, localize(26, null), { _command: error.command, _error: error.message });
            }
        });
    };
    /**
    ...
    NodeDebugSession.prototype._createStackFrame = function (frame) {
        ...
        return this._resolveValues([frame.script, frame.func, frame.receiver]).then(function () {
                        ...
                        if (PathUtils.isAbsolutePath(name_1)) {
                            ...
                            if (_this._sourceMaps) {
                                ...
                                return _this._loadScript(script_val.id).then(function (script) {
                                    if (_this._sourceMaps.HasSourceMap(script.contents)) {
            ...
            var name = localize(30, null, func_name, err.message);
            ...
            if (exists) {
                if (compareContents && (script_id || content)) {
                    return Promise.all([
                        ...
                        content ? Promise.resolve(content) : _this._loadScript(script_id).then(function (script) { return script.contents; })
                    ]).then(function (results) {
    ...
    NodeDebugSession.prototype.scopesRequest = function (response, args) {
        ...
        this._node.command2(cmd, scopesArgs).then(function (scopesResponse) {
            ...
            return Promise.all(scopes.map(function (scope) {
                ...
                if (type >= 0 && type < NodeDebugSession.SCOPE_NAMES.length) {
                    if (type === 1 && typeof scopesResponse.body.vscode_locals === 'number') {
                        expensive = true;
                        scopeName = localize(33, null, scopesArgs.maxLocals, scopesResponse.body.vscode_locals);
                    }
                    else {
                    ...
                    scopeName = localize(34, null, type);
                }
            ...
            if (frameIx === 0 && _this._exception) {
                scopes.unshift(new vscode_debugadapter_1.Scope(localize(35, null), _this._variableHandles.create(new PropertyContainer(_this._exception))));
            }
            response.body = {
                scopes: scopes
            };
    ...
    NodeDebugSession.prototype.variablesRequest = function (response, args) {
        ...
        if (variablesContainer) {
            variablesContainer.Expand(this, args.filter, args.start, args.count).then(function (variables) {
    ...
    NodeDebugSession.prototype._createArrayVariable = function (name, array) {
        ...
        return this._getArraySize(array).then(function (pair) {
            ...
            return new vscode_debugadapter_1.Variable(name, array.className + "[" + arraySize + "]", _this._variableHandles.create(new PropertyContainer(array)), indexedSize, namedSize);
        });
    };
    ...
    NodeDebugSession.prototype.pauseRequest = function (response, args) {
        ...
        this._node.command('suspend', null, function (nodeResponse) {
            if (nodeResponse.success) {
                ...
                _this._lastStoppedEvent = new vscode_debugadapter_1.StoppedEvent(localize(36, null), NodeDebugSession.DUMMY_THREAD_ID);
                _this.sendResponse(response);
            ...
            if (resp.success) {
                _this._createVariable('evaluate', resp.body).then(function (v) {
                    if (v) {
                        ...
                        response.message = localize(37, null);
                    }
                    _this.sendResponse(response);
                });
            }
                ...
                if (resp.message.indexOf('ReferenceError: ') === 0 || resp.message === 'No frames') {
                    ...
                    response.message = localize(39, null, m);
                }
                else {
        ...
        if (srcSource) {
            ...
            if (srcSource.scriptId) {
                this._loadScript(srcSource.scriptId).then(function (script) {
                    ...
                    _this.sendErrorResponse(response, 2026, localize(40, null));
                });
                return;
            }
        }
    ...
    NodeDebugSession.prototype.sendAttributeMissingErrorResponse = function (response, attribute) {
        this.sendErrorResponse(response, 2005, localize(41, null, attribute));
    };
    /**
