...
var ChromeDebugAdapter = (function () {
    ...
    ChromeDebugAdapter.prototype.launch = function (args) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            require(path.join(args.kha, 'Tools/khamake/out/main.js')).run(options, {
                ...
                //    chromeDir = chromePath.substring(0, chromePath.lastIndexOf('\\'));
                // Use custom electron
                var chromeDir = path.join(__dirname, '..', '..', '..', 'node_modules', 'electron', 'dist');
                var chromePath = chromeDir;
                if (process.platform === 'win32')
                    chromePath = path.join(chromePath, 'electron.exe');
                else if (process.platform === 'darwin')
                    chromePath = path.join(chromePath, 'Electron.app', 'Contents', 'MacOS', 'Electron');
                else
                    chromePath = path.join(chromePath, 'electron');
    ...
    ChromeDebugAdapter.prototype._attach = function (port, targetUrl, address) {
        var _this = this;
        ...
        if (!this._chromeConnection.isAttached) {
            ...
            return this._chromeConnection.attach(address, port, targetUrl).then(function () { return _this.fireEvent(new vscode_debugadapter_1.InitializedEvent()); }, function (e) {
                _this.clearEverything();
                return utils.errP(e);
            });
        }
    ...
    ChromeDebugAdapter.prototype.variables = function (args) {
        ...
        return Promise.all([
            ...
            variables.sort(function (var1, var2) { return ChromeUtils.compareVariableNames(var1.name, var2.name); });
