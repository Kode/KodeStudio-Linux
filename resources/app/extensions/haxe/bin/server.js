...
var haxeLanguageServer_Context = function(protocol) {
	...
	protocol.onVSHaxeDidChangeDisplayConfigurationIndex = $bind(this,this.onDidChangeDisplayConfigurationIndex);
};
haxeLanguageServer_Context.__name__ = ["haxeLanguageServer","Context"];
haxeLanguageServer_Context.findHaxe = function(projectDir,kha) {
...
haxeLanguageServer_Context.prototype = {
	onInitialize: function(params,token,resolve,reject) {
		...
		this.haxePath = haxeLanguageServer_Context.findHaxe(this.workspacePath,params.initializationOptions.kha);
		...
		this.haxeServer.start(this.haxePath,token,function(error) {
			if(error != null) {
				var data = { retry : false};
				var this1 = { code : 0, message : error};
				if(data != null) {
					this1.data = data;
				}
				reject(this1);
			...
			_gthis.documents = new haxeLanguageServer_TextDocuments(_gthis.protocol);
	...
	,onShutdown: function(token,resolve,_) {
	...
	,onDidChangeConfiguration: function(config) {
		...
		this.displayConfigurations = config1.displayConfigurations;
	}
...
haxeLanguageServer_HaxeServer.prototype = {
	start: function(haxePath,token,callback) {
		var _gthis = this;
		this.stop();
		this.proc = js_node_ChildProcess.spawn(haxePath,["--wait","stdio"],{ env : { HAXE_STD_PATH : js_node_Path.normalize(js_node_Path.join(haxePath,"..","std"))}});
	...
	,onExit: function(_,_1) {
		var _gthis = this;
		var message = { jsonrpc : "2.0", method : "vshaxe/log"};
		message.params = "Haxe process was killed, restarting...\n";
		this.context.protocol.writeMessage(message);
		this.proc.removeAllListeners();
		this.start(this.context.haxePath,new jsonrpc__$CancellationToken_CancellationTokenImpl(),function(error) {
			if(error != null) {
				var params = { type : 1, message : error};
	...
	,process: function(args,token,stdin,callback,errback) {
		...
		this.callbacks.push(function(data) {
			...
			while(_g1 < _g11.length) {
				...
				case 1:
					haxe_Log.trace("Haxe print:\n" + StringTools.replace(line.substring(1),"\x01","\n"),{ fileName : "HaxeServer.hx", lineNumber : 125, className : "haxeLanguageServer.HaxeServer", methodName : "process"});
					break;
				case 2:
					hasError = true;
					break;
...
haxeLanguageServer_Main.main = function() {
	...
	var protocol = new haxeLanguageServer_vscodeProtocol_Protocol(($_=new jsonrpc_node_MessageWriter(process.stdout),$bind($_,$_.write)));
...
haxeLanguageServer_features_Feature.prototype = {
	...
	,callDisplay: function(args,stdin,token,callback,errback) {
		...
		actualArgs = actualArgs.concat(_this.displayConfigurations[_this.displayConfigurationIndex]);
...
haxeLanguageServer_features_CompletionFeature.toplevelKindToCompletionItemKind = function(kind) {
	...
	case "type":
		return 7;
	default:
		haxe_Log.trace("unknown toplevel item kind: " + kind,{ fileName : "CompletionFeature.hx", lineNumber : 91, className : "haxeLanguageServer.features.CompletionFeature", methodName : "toplevelKindToCompletionItemKind"});
		return null;
	}
};
...
haxeLanguageServer_features_CompletionFeature.fieldKindToCompletionItemKind = function(kind) {
	...
	case "var":
		return 5;
	default:
		haxe_Log.trace("unknown field item kind: " + kind,{ fileName : "CompletionFeature.hx", lineNumber : 130, className : "haxeLanguageServer.features.CompletionFeature", methodName : "fieldKindToCompletionItemKind"});
		return null;
	}
};
	...
	getDiagnostics: function(uri) {
		...
		this.callDisplay(["--display",doc.fsPath + "@0@diagnostics"],null,new jsonrpc__$CancellationToken_CancellationTokenImpl(),function(s) {
			...
			try {
				...
				haxe_CallStack.lastException = e;
				if (e instanceof js__$Boot_HaxeError) e = e.val;
				haxe_Log.trace("Error parsing diagnostics response: " + Std.string(e),{ fileName : "DiagnosticsFeature.hx", lineNumber : 75, className : "haxeLanguageServer.features.DiagnosticsFeature", methodName : "getDiagnostics"});
				return;
			}
	...
	,onFindReferences: function(params,token,resolve,reject) {
		...
		this.callDisplay(["--display","" + doc.fsPath + "@" + doc.offsetToByteOffset(doc.offsetAt(params.position)) + "@usage"],doc.saved?null:doc.content,token,function(data) {
			...
			while(_g1 < _g.length) {
				...
				if(location == null) {
					haxe_Log.trace("Got invalid position: " + pos,{ fileName : "FindReferencesFeature.hx", lineNumber : 33, className : "haxeLanguageServer.features.FindReferencesFeature", methodName : "onFindReferences"});
					continue;
				}
	...
	,onGotoDefinition: function(params,token,resolve,reject) {
		...
		this.callDisplay(["--display","" + doc.fsPath + "@" + doc.offsetToByteOffset(doc.offsetAt(params.position)) + "@position"],doc.saved?null:doc.content,token,function(data) {
			...
			while(_g1 < _g.length) {
				...
				if(location == null) {
					haxe_Log.trace("Got invalid position: " + pos,{ fileName : "GotoDefinitionFeature.hx", lineNumber : 33, className : "haxeLanguageServer.features.GotoDefinitionFeature", methodName : "onGotoDefinition"});
					continue;
				}
				results.push(location);
			}
...
jsonrpc_Protocol.prototype = {
	...
	,handleRequest: function(request) {
		...
		if(__map_reserved[tokenKey] != null) {
			_this.setReserved(tokenKey,tokenSource);
		} else {
			_this.h[tokenKey] = tokenSource;
		}
		try {
			this.processRequest(request,tokenSource,resolve,reject);
		} catch( e ) {
			haxe_CallStack.lastException = e;
			if (e instanceof js__$Boot_HaxeError) e = e.val;
	...
	,processRequest: function(request,cancelToken,resolve,reject) {
...
var haxeLanguageServer_vscodeProtocol_Protocol = function(writeMessage) {
	jsonrpc_Protocol.call(this,writeMessage);
};
haxeLanguageServer_vscodeProtocol_Protocol.__name__ = ["haxeLanguageServer","vscodeProtocol","Protocol"];
haxeLanguageServer_vscodeProtocol_Protocol.__super__ = jsonrpc_Protocol;
haxeLanguageServer_vscodeProtocol_Protocol.prototype = $extend(jsonrpc_Protocol.prototype,{
	logError: function(message) {
	...
	,onVSHaxeLog: function(params) {
	}
	,onVSHaxeDidChangeDisplayConfigurationIndex: function(params) {
	}
	,processRequest: function(request,token,resolve,reject) {
		...
		case "workspace/symbol":
			...
			break;
		default:
			jsonrpc_Protocol.prototype.processRequest.call(this,request,token,resolve,reject);
		}
	}
	,processNotification: function(notification) {
		...
		case "textDocument/publishDiagnostics":
			...
			break;
		case "vshaxe/didChangeDisplayConfigurationIndex":
			this.onVSHaxeDidChangeDisplayConfigurationIndex(notification.params);
			break;
		case "vshaxe/log":
			this.onVSHaxeLog(notification.params);
			break;
	...
	,__class__: haxeLanguageServer_vscodeProtocol_Protocol
});
