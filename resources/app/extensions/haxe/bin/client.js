...
var vshaxe_DisplayConfiguration = function(context) {
	...
	this.statusBarItem = Vscode.window.createStatusBarItem(vscode__$StatusBarAlignment_StatusBarAlignment_$Impl_$.Left);
...
vshaxe_DisplayConfiguration.prototype = {
	...
	,selectConfiguration: function() {
		...
		while(_g1 < _g) {
			...
			var label = args.join(" ");
			items.push({ label : "" + index, description : label, index : index});
		}
	...
	,updateStatusBarItem: function() {
		...
		if(Vscode.languages.match({ language : "haxe", scheme : "file"},Vscode.window.activeTextEditor.document) > 0) {
			...
			if(configs != null && configs.length >= 2) {
				var index = this.context.workspaceState.get("haxe.displayConfigurationIndex",0);
				var tmp = "$(gear) Haxe: " + index + " (" + configs[index].join(" ");
				this.statusBarItem.text = tmp + ")";
...
vshaxe_InitProject.prototype = {
	...
	,scaffoldEmpty: function(root) {
		var scaffoldSource = this.context.asAbsolutePath("./scaffold");
		this.copyRec(scaffoldSource,root);
	...
	,showConfigureHint: function() {
		...
		var tasks = js_node_Fs.readFileSync(this.context.asAbsolutePath("./scaffold/.vscode/tasks.json"),{ encoding : "utf8"});
		content = StringTools.replace(content,"{{tasks}}",tasks);
		channel.clear();
	...
	,createWorkspaceConfiguration: function(vscodeDir,hxmls) {
		...
		pick.then(function(s) {
			...
			var tmp = _gthis.context.asAbsolutePath("./scaffold/.vscode");
			_gthis.copyRec(tmp,vscodeDir);
			...
			content = StringTools.replace(content,"[\"build.hxml\"],","[\"" + hxmlPath + "\"]" + (hxmls.length > 1?",":""));
			if(hxmls.length > 1) {
...
var vshaxe_Main = function(ctx) {
	...
	this.registerCommand("restartLanguageServer",$bind(this,this.restartLanguageServer));
	this.registerCommand("applyFixes",$bind(this,this.applyFixes));
	this.registerCommand("showReferences",$bind(this,this.showReferences));
	this.registerCommand("runGlobalDiagnostics",$bind(this,this.runGlobalDiagnostics));
	var wordPattern = "(-?\\d*\\.\\d\\w*)|([^\\`\\~\\!\\@\\#\\%\\^\\&\\*\\(\\)\\-\\=\\+\\[\\{\\]\\}\\\\\\|\\;\\:\\'\\\"\\,\\.\\<\\>\\/\\?\\s]+)" + "|(@:\\w*)";
	Vscode.languages.setLanguageConfiguration("Haxe",{ wordPattern : new RegExp(wordPattern)});
...
vshaxe_Main.prototype = {
	registerCommand: function(command,callback) {
		this.context.subscriptions.push(Vscode.commands.registerCommand("haxe." + command,callback));
	}
	...
	,runGlobalDiagnostics: function() {
		this.client.sendNotification({ method : "vshaxe/runGlobalDiagnostics"});
	}
	,startLanguageServer: function() {
		...
		var clientOptions = this.displayConfig.context.workspaceState.get("haxe.displayConfigurationIndex",0);
		var clientOptions1 = vshaxe_Main.findKha();
		var clientOptions2 = { documentSelector : "haxe", synchronize : { configurationSection : "haxe"}, initializationOptions : { displayConfigurationIndex : clientOptions, kha : clientOptions1}};
		this.client = new LanguageClient("haxe","Haxe",serverOptions,clientOptions2);
		this.client.logFailedRequest = function(type,error) {
			_gthis.client.warn("Request " + type.method + " failed.",error);
		};
		this.client.onReady().then(function(_) {
			_gthis.client.outputChannel.appendLine("Haxe language server started with Kha at " + vshaxe_Main.findKha());
			_gthis.displayConfig.onDidChangeIndex = function(index) {
				_gthis.client.sendNotification({ method : "vshaxe/didChangeDisplayConfigurationIndex"},{ index : index});
			};
		...
		this.serverDisposable = this.client.start();
