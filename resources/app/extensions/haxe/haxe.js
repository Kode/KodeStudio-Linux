...
HaxeContext.prototype = {
		...
		return new Promise(function(resolve,reject) {
			...
			onData = function(data) {
				...
				if(data.isServerAvailable) {
					...
					if(_g.haxeProcess != null) {
						_g.haxeProcess.kill("SIGKILL");
					}
	...
	,applyDiagnostics: function(message) {
		...
		while(tmp1.hasNext()) {
			...
			if(ds.lastDiagnostic > this.maxLastDiagnoseTime) {
				this.maxLastDiagnoseTime = ds.lastDiagnostic;
			}
		}
	}
	,getTime: function() {
	...
	,changePatchs: function(events) {
		...
		while(_g1 < events.length) {
			...
			if(!this.client.isServerAvailable) {
				if(__map_reserved[path] != null?done.getReserved(path):done.h[path]) {
					continue;
				}
				if(__map_reserved[path] != null) {
					done.setReserved(path,true);
				} else {
					done.h[path] = true;
				}
				var bl = js_node_buffer_Buffer.byteLength(text);
				if(document.isDirty) {
					patcher["delete"](0,-1).insert(0,bl,text);
				} else {
					patcher.remove();
				}
				changed = true;
				...
				while(_g11 < changes.length) {
					...
					if(rl > 0) {
						patcher["delete"](rs,rl,"c");
					}
					var text1 = change.text;
					if(text1 != "") {
						patcher.insert(rs,text1.length,text1,"c");
					}
				}
				var pos = 0;
				if(editor != null) {
					if(editor.document == document) {
						...
						if(char_pos == text.length) {
							pos = js_node_buffer_Buffer.byteLength(text);
						} else {
							pos = js_node_buffer_Buffer.byteLength(HxOverrides.substr(text,0,char_pos));
						}
					} else {
						pos = js_node_buffer_Buffer.byteLength(text);
					}
		...
		if(changed) {
			this.client.sendAll(function(s,message,error) {
				if(error == null) {
					_g.applyDiagnostics(message);
				}
...
features_CompletionHandler.prototype = {
			...
			while(i < len) {
				...
				if(features_CompletionHandler.reI.match(data)) {
					...
					case "method":
						var ts = t.split("->");
						var l = ts.length;
						if(features_CompletionHandler.reMethod.match(ts[l - 1])) {
	...
	,provideCompletionItems: function(document,position,cancelToken) {
		...
		if(!isDot) {
			displayMode = haxe_DisplayMode.Position;
		}
		...
		return new Promise(function(resolve1) {
			...
			make_request = function() {
				...
				client.sendAll(function(s,message,err) {
					if(err != null) {
						...
						resolve1(_g.parse_items(message));
					}
				},true);
			};
			...
			var doRequest = function() {
				...
				} else if(isDirty && isServerAvailable) {
				...
				client.sendAll(function(s1,message1,err1) {
					...
					if(client.isServerAvailable) {
						if(message1.severity == 2) {
							if(message1.stderr.length > 1) {
								isPatchAvailable1 = haxe_HaxeClient.isOptionExists("--patch",message1.stderr[1]);
							}
						} else {
							isPatchAvailable1 = true;
						}
					}
	...
	provideDefinition: function(document,position,cancelToken) {
		...
		return new Promise(function(resolve) {
			...
			var doRequest = function() {
				...
				var isServerAvailable = client.isServerAvailable;
				if(isPatchAvailable) {
					changeDebouncer.whenDone(make_request1);
				} else if(isDirty && isServerAvailable) {
					document.save().then(function(saved) {
						if(saved) {
							make_request1();
	...
	,setStatus: function(message,error) {
		...
		if(this.isServerAvailable) {
			...
			if(message.stderr.length > 0) {
				this.isHaxeServer = haxe_HaxeClient.reVersion.match(message.stderr[0]);
			}
		}
		return this;
	}
...
haxe_HaxeCmdLine.prototype = {
	...
	,beginPatch: function(fileName) {
		...
		if(__map_reserved[fileName] != null) {
			tmp = _this.getReserved(fileName);
		} else {
			tmp = _this.h[fileName];
		}
		var tmp1 = tmp;
		if(tmp1 == null) {
			tmp1 = new haxe_HaxePatcherCmd(fileName);
		}
		var _this1 = this.patchers;
		if(__map_reserved[fileName] != null) {
			_this1.setReserved(fileName,tmp1);
		} else {
			_this1.h[fileName] = tmp1;
		}
		return tmp1;
	}
	,clearPatch: function() {
...
haxe_HaxeConfiguration.findHaxeExec = function(conf,projectDir,platform) {
	...
	try {
		...
		var globalPath = js_node_Path.join(haxe_HaxeConfiguration.findKha(),"Tools","Haxe");
		try {
			if(js_node_Fs.statSync(localPath).isDirectory()) {
...
haxe_HaxeConfiguration.findKha = function() {
...
platform_Platform.init = function(platformName,archName) {
	...
	if(platformName == "win32") {
		...
		if(platformName == "linux") {
			if(archName == "x64") {
				platform_Platform.instance.executableExtensionK = "-linux64";
			} else {
				platform_Platform.instance.executableExtensionK = "-linux32";
			}
		} else {
			platform_Platform.instance.executableExtensionK = "-osx";
		}
	}
